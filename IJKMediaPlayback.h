...
typedef NS_ENUM(NSInteger, IJKMPMovieScalingMode) {
    IJKMPMovieScalingModeNone,       // 原始尺寸
    IJKMPMovieScalingModeAspectFit,  // 竖向等比例缩放（高度 = 屏幕高度），视频比例不改变，可能横向有黑边
    IJKMPMovieScalingModeAspectFill, // 横向等比例缩放（宽度 = 屏幕宽度），视频比例不改变，可能纵向被裁减
    IJKMPMovieScalingModeFill        // 全屏填充（可能会被拉伸）
};
