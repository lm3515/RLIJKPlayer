...
int rtspRecordVideo(const char *streamUrl, const char *path, bool bStop){
    ...
    // 打开网络流或文件流
    if (avformat_open_input(&i_fmt_ctx, streamUrl, NULL, NULL)!=0)
    {
    ...
    // 列出输入文件的相关流信息
    printf("---------------- 输入文件信息 ---------------\n");
    av_dump_format(i_fmt_ctx, 0, streamUrl, 0);
    printf("-------------------------------------------------\n");
    ...
    // 初始化输出 o_fmt_ctx 结构体
    if (avformat_alloc_output_context2(&o_fmt_ctx, NULL, NULL, path)!=0){
    ...
    o_video_stream = avformat_new_stream(o_fmt_ctx, NULL);
    {
        ...
        oVcc->sample_aspect_ratio.num = i_video_stream->time_base.num;
        oVcc->sample_aspect_ratio.den = i_video_stream->time_base.den;
        fprintf(stderr, "time_base.num = %d time_base.den = %d\n", oVcc->sample_aspect_ratio.num, oVcc->sample_aspect_ratio.den);
        ...
        printf("%d %d %d", oVcc->width, oVcc->height, oVcc->format);
        
        oVcc->codec_tag = i_video_stream->codecpar->codec_tag;
        oVcc->field_order = i_video_stream->codecpar->field_order;
        
        oVcc->level = i_video_stream->codecpar->level;
        oVcc->profile = i_video_stream->codecpar->profile;
    ...
    // 列出输出文件的相关流信息
    printf("------------------- 输出文件信息 ------------------\n");
    av_dump_format(o_fmt_ctx, 0, path, 1);
    printf("-------------------------------------------------\n");
    
    avio_open(&o_fmt_ctx->pb, path, AVIO_FLAG_WRITE);
    
    // 根据文件名的后缀写相应格式的文件头
    int write_i = avformat_write_header(o_fmt_ctx, NULL);
    printf("%d", write_i);
